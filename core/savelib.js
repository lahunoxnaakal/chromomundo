...
var cm_SaveLib =
{
    saveData: function saveData(name,data)
    {
        var exdate=new Date();
    ...
    loadData: function loadData(name,def)
    {
    ...
    getNameValueCollection: function getNameValueCollection()
    {
        var parse = document.cookie;
        var dict = {};
        var end = false;
        while ( parse.length > 2 && (end == false) )
        {
            ...
            var value = cm_SaveLib.loadData(key, "");
            if ( l_value > 0 )
